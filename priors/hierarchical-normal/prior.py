...
class HierarchicalNormalPrior(Prior):
    def sample(self, *, seed, num_groups=8, per_group=12,
               population_std=1.0, group_std=0.5, **_) -> dict[str, Any]:
        ...
        X = np.stack([groups.astype(np.float32),
                      np.tile(np.arange(per_group), num_groups).astype(np.float32)], axis=-1)
