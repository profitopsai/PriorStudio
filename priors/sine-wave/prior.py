...
class SineWavePrior(Prior):
    def sample(self, *, seed, num_points=100, amplitude_max=2.0,
               omega_min=0.5, omega_max=2.0, noise_scale=0.1, **_) -> dict[str, Any]:
        ...
        y = (a * np.sin(omega * t + phi) + rng.normal(0, noise_scale, size=num_points)).astype(np.float32)
        return {"t": t.reshape(-1, 1), "y": y, "X": y.reshape(-1, 1),
                "a_true": a, "omega_true": omega, "phi_true": phi}
