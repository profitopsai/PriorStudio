...
class PolynomialRegressionPrior(Prior):
    def sample(self, *, seed, num_points=100, max_degree=3,
               coef_scale=1.0, noise_scale=0.2, **_) -> dict[str, Any]:
        ...
        y = np.polyval(coefs[::-1], x) + rng.normal(0, noise_scale, size=num_points).astype(np.float32)
